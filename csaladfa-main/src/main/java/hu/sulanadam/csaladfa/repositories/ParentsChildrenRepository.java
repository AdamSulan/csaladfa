package hu.sulanadam.csaladfa.repositories;
...
import hu.sulanadam.csaladfa.entities.ParentsChildren;

public interface ParentsChildrenRepository extends JpaRepository<ParentsChildren, Long> {
    

    
    @Query("SELECT NEW hu.sulanadam.csaladfa.dtos.personDtos.out.PersonDTOIdName(p.parent.id, p.parent.name) FROM ParentsChildren p WHERE p.child.id = :childId")
    List<PersonDTOIdName> getParentsByChildId(@Param("childId") Long childId);
    
    @Query("SELECT NEW hu.sulanadam.csaladfa.dtos.personDtos.out.PersonDTOIdName(p.child.id, p.child.name) FROM ParentsChildren p WHERE p.parent.id = :parentId")
    List<PersonDTOIdName> getChildrenByParentId(@Param("parentId") Long parentId);
    
    @Query("SELECT new hu.sulanadam.csaladfa.dtos.personDtos.out.PersonDTOIdNameSex(p.parent.id, p.parent.name, p.parent.sex) FROM ParentsChildren p WHERE p.child.id = :childId")
